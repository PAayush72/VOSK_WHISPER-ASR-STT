...
NAME_BIAS = "Ujjval, ChatGPT, India, Republic Day, Surat, Ayush, Toshif, Nisarg, Harsh, Neeraj"
...
# def send_ws(message: dict):
#     if ws_loop and ws_loop.is_running():
#         asyncio.run_coroutine_threadsafe(
#             ws_broadcast(json.dumps(message)),
#             ws_loop
#         )
...
# def extract_keywords(text, max_words=10):
#     return ", ".join([w for w in text.split() if len(w) > 2][:max_words])
...
#     while True:
#         index, audio_bytes, rough_text = polish_queue.get()
#         tmp_file = f"temp_{index}.wav"
...
#             prompt = (
#                 f"Context: {extract_keywords(rough_text)}. "
#                 f"Names likely: {NAME_BIAS}. "
#                 "Spoken dictation."
#             )
...
#             polished = " ".join(s.text.strip() for s in segments)

#             if polished:
#                 print(f"\n✅ {polished}")
#                 send_ws({"type": "final", "text": polished})

#                 if check_exit_command(polished):
#                     _thread.interrupt_main()
...
#         buffer_time = len(sentence_audio_buffer) * CHUNK / RATE

#         if silence > SILENCE_THRESHOLD or buffer_time > MAX_BUFFER_SECONDS:
#             result = json.loads(recognizer.FinalResult())
#             text = result.get("text", "").strip()
#             recognizer.Reset()
...
#                     is_processing = True
#                 audio_bytes = b"".join(sentence_audio_buffer)
#                 polish_queue.put((len(committed_sentences), audio_bytes, text))
#             sentence_audio_buffer.clear()
#             last_speech_time = time.time()
